...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/setWebhook") {
      const r = await fetch("https://api.telegram.org/bot" + env.BOT_TOKEN + "/setWebhook?url=" + url.origin + "/webhook");
    ...
    if (url.pathname === "/testProxy") {
      try {
        const result = await fetchViaProxy(env, "https://www.microsoft.com/software-download/windows11", BH);
    ...
    if (url.pathname === "/testMS") {
      ...
      const testUrl = "https://www.microsoft.com/software-download-connector/api/getskuinformationbyproductedition"
        + "?profile=" + PROFILE_ID
        + "&productEditionId=2618&SKU=undefined&friendlyFileName=undefined"
        + "&Locale=en-US&sessionID=" + sid;
      ...
      try {
        ...
        const out = "DIRECT: " + directText.slice(0, 300) + "\n\nPROXY: " + proxyText.slice(0, 300);
    ...
    if (url.pathname === "/testFull") {
      ...
      try {
        log.push("1. preVisit...");
        await preVisit("https://www.microsoft.com/software-download/windows10ISO", env);
        log.push("   OK");
        ...
        const apiUrl = "https://www.microsoft.com/software-download-connector/api/getskuinformationbyproductedition"
          + "?profile=" + PROFILE_ID
          + "&productEditionId=2618&SKU=undefined&friendlyFileName=undefined"
          + "&Locale=en-US&sessionID=" + sid;
        const ref = "https://www.microsoft.com/software-download/windows10ISO";
        ...
        const proxyResult = await fetchViaProxy(env, apiUrl, { ...AH, Referer: ref });
    ...
    // ── Полный тест сессии для Windows 11 ──
    if (url.pathname === "/testWin11") {
      const log = [];
      try {
        const editionIds = [3321, 3324];
        log.push("Testing Windows 11 session with editionIds: " + JSON.stringify(editionIds));

        for (let si = 0; si < editionIds.length; si++) {
          const sid = crypto.randomUUID();
          log.push("\n--- editionId=" + editionIds[si] + " sid=" + sid.slice(0, 8) + "...");
          ...
          const apiUrl = "https://www.microsoft.com/software-download-connector/api/getskuinformationbyproductedition"
            + "?profile=" + PROFILE_ID
            + "&productEditionId=" + editionIds[si]
            + "&SKU=undefined&friendlyFileName=undefined&Locale=en-US&sessionID=" + sid;

          const msResult = await msFetch(env, apiUrl, { ...AH, Referer: "https://www.microsoft.com/software-download/windows11" }, c2);
          log.push("  getsku response(200): " + msResult.text.slice(0, 200));
          log.push("  is715: " + is715(msResult.text));

          // Парсим и пробуем GetProductDownloadLinksBySku для первого SKU
          try {
            ...
            if (data.Errors && data.Errors.length > 0) {
              ...
              log.push("  First SKU: id=" + sku.Id + " lang=" + sku.Language);

              const dlUrl = "https://www.microsoft.com/software-download-connector/api/GetProductDownloadLinksBySku"
                + "?profile=" + PROFILE_ID
                + "&productEditionId=undefined&SKU=" + sku.Id
                + "&friendlyFileName=undefined&Locale=en-US&sessionID=" + sid;

              const dlResult = await msFetch(env, dlUrl, { ...AH, Referer: "https://www.microsoft.com/software-download/windows11" }, msResult.cookies);
              log.push("  GetLinks response(200): " + dlResult.text.slice(0, 200));

              try {
                const dlData = JSON.parse(dlResult.text);
                if (dlData.Errors && dlData.Errors.length > 0) {
                  log.push("  LINKS ERRORS: " + JSON.stringify(dlData.Errors[0]));
                } else if (dlData.ProductDownloadOptions) {
                  log.push("  LINKS OK: " + dlData.ProductDownloadOptions.length + " options");
                }
              } catch (pe) {
  ...
  console.log("[webhook] received update_id:", update?.update_id, "has_callback:", !!update?.callback_query, "has_message:", !!update?.message);
  ctx.waitUntil(
    handleUpdate(update, env, ctx).catch(e => console.error("[handleUpdate] uncaught:", String(e)))
  );
  return new Response("OK");
}
